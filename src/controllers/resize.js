...
function luckysheetsizeauto(isRefreshCanvas=true) {
    $("#" + Store.container).find(".luckysheet-grid-container").css("top", 0 ); // Store.toolbarHeight );
...
function changeSheetContainerSize(gridW, gridH){
    ...
    $("#luckysheet-scrollbar-y").height(cellmainHeight + Store.columnHeaderHeight - Store.cellMainSrollBarSize - 3);
    $("#luckysheet-scrollbar-y").width(Store.cellMainSrollBarSize);
    $("#luckysheet-scrollbar-x").height(Store.cellMainSrollBarSize);
    $("#luckysheet-scrollbar-x").width(cellmainWidth).css("left", Store.rowHeaderWidth - 2);
