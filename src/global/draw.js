...
import { getRealCellValue, checkstatusByCell } from "./getdata";
...
function getCellTextInfo(cell, ctx, option) {
  ...
  let textActualBoundingBoxDescent =
    ctx.measureText(value).actualBoundingBoxDescent * Store.zoomRatio;
  let textActualBoundingBoxAscent =
    ctx.measureText(value).actualBoundingBoxAscent * Store.zoomRatio;
  let textHeight = textActualBoundingBoxDescent + textActualBoundingBoxAscent;

  let height =
    textHeight + textHeight / 2 - textActualBoundingBoxDescent - space_height;
...
function fillABC123(txt, rectOption, txtOption, luckysheetTableContent) {
  ...
  luckysheetTableContent.fillText(
    txt,
    txtOption.horizonAlignPos / Store.zoomRatio,
    txtOption.verticalAlignPos / Store.zoomRatio
  );
